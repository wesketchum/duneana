...
void do_frugal_update_absRS(short& median, int& runningDiff, const short sample, const int ncontigabs, const int increment)
{
    if(sample>median) ++runningDiff;
    if(sample<median) --runningDiff;
    
    if(runningDiff > ncontigabs){
        median += increment;
        runningDiff=0;
    }
    if(runningDiff < -1*ncontigabs){
        median -= increment;
        runningDiff=0;
    }
}
...
std::vector<short> frugal_iqr_absRS(const std::vector<short>& raw_in,
                              const std::vector<short>& median,
                              const int ncontigabs)
{
    std::vector<short> iqr(raw_in.size(), 0);
    ...
    for(size_t i=0; i<raw_in.size(); ++i){
        ...
        if(s<m)
	  do_frugal_update_absRS(quartileLo, runningDiffLo, s, ncontigabs, increment);
        if(s>m)
	  do_frugal_update_absRS(quartileHi, runningDiffHi, s, ncontigabs, increment);
